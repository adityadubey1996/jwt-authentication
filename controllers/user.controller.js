...
async function GenerateRefreshToken(user_id) {
    ...
    const refresh_token = jwt.sign({ sub: user_id }, process.env.JWT_REFRESH_TOKEN, { expiresIn: process.env.JWT_REFRESH_TIME });
    try{
  const res =  await redis_client.get(user_id.toString(),async (err, data) => {
        if(err) throw err;

       const response = await redis_client.set(user_id.toString(), JSON.stringify({token: refresh_token}));
       console.log(`response is ${response}`)
    })
    console.log(`res is ${res}`);
}
catch(e){
    console.log(`error here with error ${e}`)
}
